export const BLACK = 0x000000;
export const GRAY = 0x999999;
export const WHITE = 0xffffff;

export const GRAY_LIGHT = 0xbfbfbf;
export const GRAY_LIGHTER = 0xd9d9d9;
export const GRAY_LIGHTEST = 0xe5e5e5;
export const GRAY_DARK = 0x666666;
export const GRAY_DARKER = 0x444444;
export const GRAY_DARKEST = 0x222222;

export const CYAN = 0x3ae0e5;
export const GREEN = 0x40bf00;
export const ORANGE = 0xff5500;
export const PINK = 0xec3bb6;
export const RED = 0xf60029;
export const YELLOW = 0xffcc00;
export const BLUE = 0x4e40ff;

export const PRIMARY = ORANGE;
export const SECONDARY = BLUE;

export const LOW_QUALITY_BLUE = 0xdcd9ff;
export const LOW_QUALITY_BLUE_ARR = [0.862745098, 0.850980392, 1];
export const LOW_QUALITY_BLUE_RGBA = [220, 217, 255, 255];
export const LOW_QUALITY_ORANGE = 0xffd9cb;
export const LOW_QUALITY_ORANGE_ARR = [1, 0.890196078, 0.835294118];
export const LOW_QUALITY_ORANGE_RGBA = [255, 227, 213, 255];

export default {
  BLACK,
  GRAY,
  WHITE,
  GRAY_LIGHT,
  GRAY_LIGHTER,
  GRAY_LIGHTEST,
  GRAY_DARK,
  GRAY_DARKER,
  GRAY_DARKEST,
  CYAN,
  GREEN,
  ORANGE,
  PINK,
  RED,
  YELLOW,
  BLUE,
  PRIMARY,
  SECONDARY,
  LOW_QUALITY_BLUE,
  LOW_QUALITY_BLUE_ARR,
  LOW_QUALITY_BLUE_RGBA,
  LOW_QUALITY_ORANGE,
  LOW_QUALITY_ORANGE_ARR,
  LOW_QUALITY_ORANGE_RGBA
};
